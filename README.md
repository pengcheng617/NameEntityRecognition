...
crf.dll ：通过JNI调用,依赖于libcrfpp.dll,JNI件通过Swig工具生成，dll通过Visual stdio编译生成
libCRFPP.so ：Linux系统中使用的模型
----------
档和链接
---------
CRF模型介绍[1]
...
**CRFModel**: 加载CRF model，dll，so件等
**RuleFilter**: 通过添加一些规则，识别常地址串，通过CRF模型识别出来的地址字段较短，1法识别地址后面“**街道”、“******路**”、“******号”之类的常地址串
